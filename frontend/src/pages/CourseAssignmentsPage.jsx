...
const CourseAssignmentsPage = () => {
  ...
  // 🛠️ Wrapped in useCallback to prevent continuous component re-renders and fix ESLint warnings
  const fetchClassroomDetails = useCallback(async () => {
    ...
    try {
      ...
      if (courseData.assignments && courseData.assignments.length > 0) {
        setSelectedAssignment(courseData.assignments[0]);
      }
      setLoading(false);
    } catch (err) {
      console.error("Error loading classroom details:", err);
      setMessage({ type: 'error', text: 'Failed to retrieve course assignments.' });
      setLoading(false);
    }
  }, [id]);
  ...
  const handleCreateAssignment = async (e) => {
    ...
    try {
      ...
      const res = await apiClient.post(`/courses/${id}/assignments`, newAssignment, config);
      if (res.data.success) {
        ...
        if (res.data.data && res.data.data.length > 0) {
          // Select latest assignment
          setSelectedAssignment(res.data.data[res.data.data.length - 1]);
        }
      }
    } catch (err) {
  ...
  const handleDeleteAssignment = async (assignmentId) => {
    if (!window.confirm("Are you sure you want to delete this assignment? All recorded grades will be cleared.")) {
      return;
    }
    ...
    try {
      ...
      const res = await apiClient.delete(`/courses/${id}/assignments/${assignmentId}`, config);
      if (res.data.success) {
  ...
  const handleGradeChange = async (studentId, assignmentId, marksVal) => {
    try {
      const marks = Number(marksVal) || 0;

      await apiClient.put(`/courses/${id}/grades/assignment`, {
        studentId,
        assignmentId,
        marks
      });
      ...
      setStudentGrades(prevGrades => 
        prevGrades.map(sg => {
          if (sg.studentId === studentId) {
            const marksIndex = sg.assignmentMarks.findIndex(am => am.assignmentId === assignmentId);
            const freshMarks = [...sg.assignmentMarks];
            if (marksIndex > -1) {
              freshMarks[marksIndex] = { ...freshMarks[marksIndex], marks };
            } else {
              freshMarks.push({ assignmentId, marks, status: 'Graded' });
            }
            return { ...sg, assignmentMarks: freshMarks };
          }
          return sg;
        })
      );
    } catch (err) {
      console.error("Failed to update student assignment mark:", err);
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6 font-sans animate-fade-in">
      ...
      <div className="flex gap-2 border-b border-slate-200 pb-px mb-8 overflow-x-auto">
        ...
        <button className="border-b-2 border-blue-600 px-4 py-2.5 text-sm font-bold text-blue-600 whitespace-nowrap">
          📝 Homework Logs ({assignments.length})
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Left column: Assign New Task / Assignments List */}
        <div className="flex flex-col gap-6 h-fit">
          {/* Creator form */}
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <h3 className="text-lg font-bold text-slate-800 mb-4">Post New Assignment</h3>
            <form onSubmit={handleCreateAssignment} className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Assignment Title</label>
                <input 
                  type="text" 
                  ...
                  placeholder="e.g. Project 1 Buildout Core Stack" 
                  required 
              ...
              <button 
                ...
                disabled={actionLoading}
              >
                {actionLoading ? 'Publishing...' : 'Assign Homework'}
              </button>
            </form>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-3">
            <h3 className="text-base font-bold text-slate-800">Assignments Inventory</h3>
            {assignments.length === 0 ? (
              <p className="text-slate-400 text-xs text-center py-6 bg-slate-50 rounded-xl border border-dashed">No assignments posted.</p>
            ) : (
                  ...
                  >
                    <div className="truncate max-w-[150px]">
                      ...
                      <p className="text-[10px] text-slate-400 mt-0.5">
                        Due: {ass.deadline ? new Date(ass.deadline).toLocaleDateString() : 'No limit'}
                      </p>
                    </div>
                    <button
                      onClick={(e) => {
          ...
          {selectedAssignment ? (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-5">
              <div className="bg-slate-900 text-white p-6 rounded-xl shadow-sm">
                <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Active Evaluated Task Vector</span>
                <h3 className="text-xl font-bold mt-0.5">{selectedAssignment.title}</h3>
                {selectedAssignment.description && (
                  <p className="text-xs text-slate-400 mt-2 italic leading-relaxed">{selectedAssignment.description}</p>
                )}
                {selectedAssignment.deadline && (
                  <p className="text-xs text-rose-400 font-bold uppercase tracking-widest mt-3">
                    Cut-off Deadline: {new Date(selectedAssignment.deadline).toLocaleDateString()}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {studentGrades.length === 0 ? (
                  <p className="p-6 text-slate-400 text-sm text-center bg-slate-50 rounded-xl border border-dashed">
                    No students are currently enrolled to receive marks.
                  </p>
                ) : (
                    ...
                    <table className="w-full text-left text-sm min-w-[500px]">
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                        {studentGrades.map(sg => {
                          const currentMarkObj = sg.assignmentMarks?.find(am => am.assignmentId === selectedAssignment._id);
                              ...
                              <td className="p-4">
                                <div className="flex items-center gap-2">
                                  <input 
                                    ...
                                    min="0"
                                    value={currentMark}
                                    onChange={(e) => handleGradeChange(sg.studentId, selectedAssignment._id, e.target.value)}
            ...
            <div className="bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center text-slate-400 font-medium h-full flex flex-col justify-center items-center">
              <span>📝</span>
              <p className="mt-2 text-sm">Please select or create an assignment from the inventory panel to manage grading.</p>
            </div>
          )}
        </div>
